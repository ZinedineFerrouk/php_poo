...
class Form
{
     ...
     * @return string
     */
    public function inputNum($name, $data = null)
    {
        return $this->arround('<input type="number" id="' . $name . '" name="' . $name . '" value="' . $this->getValue($name, $data) . '">');
    }
     ...
     * @return string
     */
    public function select($name, $entitys, $column, $tre, $data = '', $id = 'id')
    {
        $html = '<select name="' . $name . '">';
        foreach ($entitys as $entity) {
            if (!empty($data) && $data == $entity->$id) {
                $selected = ' selected="selected"';
            } else {
                $selected = '';
            }
            $html .= '<option value="' . $entity->$id . '"' . $selected . '>' . $entity->$column . ' - ' . $entity->$tre . '</option>';
        }
        $html .= '</select>';
        return $html;
    }
